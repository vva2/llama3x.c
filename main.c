...
void read_checkpoint(char* checkpoint, Config* config, TransformerWeights* weights, int* fd, float** data, ssize_t* file_size)
{
    ...
    {
        fprintf(stderr, "read failed! error in reading config\n");
        exit(EXIT_FAILURE);
    }
  ...
  *file_size = ftell(file); // get the file size, in bytes
    ...
    *fd = open(checkpoint, O_RDONLY); // open in read only mode
    if (*fd == -1)
    {
        fprintf(stderr, "open failed! error in reading checkpoint\n");
        exit(EXIT_FAILURE);
    }
...
void build_tokenizer(Tokenizer* t, char* tokenizer_path, int vocab_size) {
    ...
    for (int i = 0; i < vocab_size; i++) {
        ...
        if(strcmp(t->vocab[i], "<|begin_of_text|>") == 0)
        {
            t->BOS_TOKEN_ID = i;
        }
        else if(strcmp(t->vocab[i], "<|end_of_text|>") == 0)
        {
            t->EOS_TOKEN_ID = i;
        }
    }
...
char* decode(Tokenizer* t, int prev_token, int token) {
    ...
    if (prev_token == t->BOS_TOKEN_ID && piece[0] == ' ') { piece++; }
